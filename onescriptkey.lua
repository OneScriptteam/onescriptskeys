...
if (not getgenv) or (getgenv and type(getgenv) ~= "function") then CheatEngineMode = false end
if getgenv and not getgenv().shared then CheatEngineMode = false; getgenv().shared = {}; end
if getgenv and not getgenv().debug then CheatEngineMode = false; getgenv().debug = {traceback = function(string) return string end} end
if getgenv and not getgenv().require then CheatEngineMode = false; end
if getgenv and getgenv().require and type(getgenv().require) ~= "function" then CheatEngineMode = false end
local debugChecks = {
    Type = "table",
    Functions = {
        "getupvalue",
    ...
    if identifyexecutor ~= nil and type(identifyexecutor) == "function" then
        ...
        if suc then
            for i,v in pairs(blacklist) do
                if string.find(string.lower(tostring(res)), v) then CheatEngineMode = false end
            end
...
if IS_DOWN and not shared.BYPASS_VW_PROTECTION then
    ...
    if not suc then shared.CheatEngineMode = false end
    ...
    if (shared.CheatEngineMode or not verified) and not shared.AcceptedRisksOfBan then
        ...
        shared.AcceptedRisksOfBan = false
        return
    end
    ...
    task.spawn(function()
        pcall(function()
            ...
            loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeOneScript/VW-Add/"..tostring(commit).."/inkgamereducer.lua", false))()
        end)
    end)

    loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeOneScript/VW-Add/"..tostring(commit).."/newinkgame.lua", false))()
end